...
class SimpleSenseSystem {
    ...
    /**
     * 管理展开顺序
     * @param {string} senseId - 感官ID
     * @param {boolean} isExpanding - 是否正在展开
     */
    static manageExpansionOrder(senseId, isExpanding) {
        if (isExpanding) {
            // 添加到展开顺序
            if (!this.expansionOrder.includes(senseId)) {
                this.expansionOrder.push(senseId);
            }
            // 设置z-index基于展开顺序，确保优先级最高
            const orderIndex = this.expansionOrder.indexOf(senseId);
            const element = document.getElementById(`sense-${senseId}`);
            if (element) {
                element.style.setProperty('z-index', 3000 + orderIndex, 'important');
            }
        } else {
            // 从展开顺序中移除
            const index = this.expansionOrder.indexOf(senseId);
            if (index > -1) {
                this.expansionOrder.splice(index, 1);
            }
            // 重新设置所有展开元素的z-index
            this.updateExpansionZIndex();
        }
    }
     ...
     * 更新所有展开元素的z-index
     */
    static updateExpansionZIndex() {
        this.expansionOrder.forEach((senseId, index) => {
            const element = document.getElementById(`sense-${senseId}`);
            if (element) {
                element.style.setProperty('z-index', 3000 + index, 'important');
            }
        });
    }
     ...
     */
    static adjustPositionForOverlap(position, senseType) {
        if (!position) return null;
        
        const existingPositions = Array.from(this.senseInstances.values()).map(instance => ({
            x: instance.currentPosition.x,
            y: instance.currentPosition.y,
            type: instance.senseType
        }));
        
        // 检查是否有冲突（距离小于100px）
        const hasConflict = existingPositions.some(existing => {
        ...
        if (hasConflict) {
            // 如果有冲突，使用默认位置
            return this.getDefaultPosition(senseType);
        }
    ...
    static createSense(senseType, value, position = null, distance = 50, customId = null) {
        ...
        // 检测位置冲突并调整
        const adjustedPosition = this.adjustPositionForOverlap(position, senseType);
        
        console.log(`[SimpleSenseSystem] 创建五感 - ID: ${id}, 类型: ${senseType}, 位置: ${adjustedPosition ? `(${adjustedPosition.x}, ${adjustedPosition.y})` : '默认'}, 距离: ${distance}, 值: ${value}`);
        ...
        const instance = new SenseInstance(id, senseType, value, adjustedPosition, distance);
    ...
    static createSenseElement(instance) {
        ...
        // 使用实例ID作为CSS类名的一部分，确保左右触觉有不同的类名
        const cssClassName = instance.id.includes('_') ? instance.id : instance.senseType;
        element.className = `dynamic-sense ${cssClassName}-sense`;
    ...
    static addMouseEvents(element, instance) {
        ...
        element.addEventListener('click', (e) => {
            ...
            if (element.classList.contains('locked')) {
                ...
                this.manageExpansionOrder(instance.id, false);
                console.log(`[SimpleSenseSystem] 解锁五感: ${instance.id}`);
            } else {
                element.classList.add('locked');
                this.manageExpansionOrder(instance.id, true);
                console.log(`[SimpleSenseSystem] 锁定五感: ${instance.id}`);
            }
            
            // 动态调整对话位置
            if (window.DialogueSystem && window.DialogueSystem.adjustDialoguePosition) {
                setTimeout(() => {
                    window.DialogueSystem.adjustDialoguePosition();
                }, 100);
            }
        });
